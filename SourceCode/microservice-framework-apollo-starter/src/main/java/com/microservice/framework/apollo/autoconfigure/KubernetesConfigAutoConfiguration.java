 ...
 * Kubernetes 配置源自动配置
 * <p>
 * 根据 {@code framework.apollo.kubernetes.enabled} 属性决定是否激活
 * Kubernetes ConfigMap 配置源，默认不启用。
 * <p>
 * Kubernetes 配置源作为 Apollo 的补充，在云原生环境中从 ConfigMap
...
@ConditionalOnProperty(prefix = "framework.apollo.kubernetes", name = "enabled", havingValue = "true")
public class KubernetesConfigAutoConfiguration {

    /**
     * 注册 Kubernetes 配置源描述符
     * <p>
     * 默认从 {@link ApolloProperties} 中获取 ConfigMap 名称和 Namespace。
    ...
    static class KubernetesConfigSourceConfiguration {
        ...
        ConfigSourceDescriptor kubernetesConfigSourceDescriptor(ApolloProperties properties) {
            ApolloProperties.KubernetesProperties k8s = properties.getKubernetes();
